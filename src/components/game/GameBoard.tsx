...
const GameBoard = ({
  ...
  const headerBar = (
    <div className="flex flex-shrink-0 items-center justify-between border-b border-primary/15 px-2 py-1.5 md:px-4 md:py-2.5 gap-2">
      <div className="flex items-center gap-1.5 md:gap-3 shrink-0">
        <Crown className="h-4 w-4 md:h-5 md:w-5 text-primary" />
        <span className="font-mono text-xs md:text-sm tracking-widest text-primary leading-none">{roomCode}</span>
        <HowToPlayModal
          trigger={
            <button className="flex h-6 w-6 items-center justify-center rounded text-muted-foreground transition-colors hover:bg-muted hover:text-foreground" title="How to Play">
        ...
        {isDiscord && (
          <span className="flex h-6 items-center gap-1 rounded border border-[hsl(235_86%_65%/0.3)] bg-[hsl(235_86%_65%/0.1)] px-1.5 font-mono text-[10px] uppercase tracking-wider text-[hsl(235_86%_65%)]">
      ...
      <div className="flex items-center gap-1.5 md:gap-3 text-sm text-muted-foreground overflow-hidden">
        ...
        {currentRound && (
          <span className="hidden sm:flex h-6 items-center font-display text-[10px] md:text-xs uppercase tracking-wider">
            Round {currentRound.round_number}
          </span>
        )}
        <span className="flex h-6 items-center rounded border border-primary/20 bg-primary/5 px-1.5 font-display text-[10px] md:text-xs uppercase tracking-wider text-primary shrink-0">
          {phaseLabels[phase] ?? phase}
        </span>
        {myRole && (
          <span className={`flex h-6 items-center gap-1 shrink-0 rounded border px-1.5 font-display text-[10px] uppercase tracking-wider ${
        ...
        {currentPlayerId && players.length > 0 && (
          <span className="hidden md:flex h-6 items-center gap-1.5 rounded border border-border bg-muted/50 px-2 font-body text-xs text-foreground">
    ...
    <div className="noise-overlay fixed inset-0 flex flex-col">
      ...
      <div className="relative z-10 flex flex-1 flex-col min-h-0 overflow-hidden">
        ...
        <div className="flex flex-1 flex-col min-h-0 overflow-y-auto pb-20 md:hidden">
          ...
          {/* Action panel — election nominate hidden on mobile, VotingPanel + overlays still render */}
          {actionPanel && <div className="px-3 py-2 flex-shrink-0">{actionPanel}</div>}

          {/* Chronicle — capped height */}
          <div className="px-3 py-2 flex-shrink-0">
        ...
        <div className="hidden md:flex lg:hidden flex-1 flex-col min-h-0 overflow-hidden">
          {/* Top zone — trackers, council, action */}
          ...
          {/* Bottom zone — Chronicle + Chat side by side */}
          <div className="flex-1 flex flex-row min-h-0 border-t border-primary/15">
